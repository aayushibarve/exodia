...
IMG_DIR = "/home/barve/exodia/depth_test/experiment_002/images/000002"
#IMG_DIR = "/home/barve/exodia/align_frames/images/000002"
...
# --- Fast-FoundationStereo settings ---
FFS_REPO_DIR = "/home/barve/exodia/Fast-FoundationStereo"        # <-- EDIT
FFS_WEIGHTS_DIR = "/home/barve/exodia/ffs_weights/23-36-37"      # <-- EDIT: downloaded checkpoint folder
FFS_VALID_ITERS = 8      # refinement iterations; lower = faster, slightly less accurate
FFS_MAX_DISP = 192
FFS_SCALE = 1.0          # <1.0 downsizes input for speed/memory; depth is upsized back with nearest-neighbor
FFS_HIERA = 0            # 1 = hierarchical inference; repo recommends this OR FFS_SCALE<1 for >1000px images
...
# ============================================================
# FAST-FOUNDATIONSTEREO -- direct in-process port of scripts/run_demo.py (model load ->
# resize/pad -> forward -> disp -> depth), the same port used and validated in
# vo_mocap_pipeline.py. Returns (disparity, Z, valid) to match compute_stereo_depth's
# signature exactly, so it's a drop-in replacement for the rest of this pipeline -- the
# "disparity" returned here is reconstructed from the metric depth purely for the diagnostic
# plot (fx*baseline/Z), since FFS itself only needs to hand back metric depth downstream.
# ============================================================
def check_weights():
    assert os.path.isdir(FFS_WEIGHTS_DIR), (
        f"Checkpoint folder not found at {FFS_WEIGHTS_DIR}.\n"
        "Download a weights folder (e.g. '23-36-37') from the Fast-FoundationStereo README's "
        "Google Drive link, place it locally, and update FFS_WEIGHTS_DIR above."
    )
    ckpt = next((os.path.join(FFS_WEIGHTS_DIR, f) for f in os.listdir(FFS_WEIGHTS_DIR)
                 if f.endswith(".pth")), None)
    assert ckpt is not None, f"No .pth checkpoint found in {FFS_WEIGHTS_DIR}"
    print("Using checkpoint:", ckpt)
    return ckpt


def load_foundation_stereo_model(ffs_repo_dir, model_ckpt):
    """Load the model + its two helper imports, exactly as scripts/run_demo.py does."""
    import torch
    if ffs_repo_dir not in sys.path:
        sys.path.insert(0, ffs_repo_dir)
    from core.utils.utils import InputPadder
    from Utils import AMP_DTYPE

    torch.autograd.set_grad_enabled(False)  # inference only, no grad needed anywhere
    ...
    model.cuda().eval()
    print("Fast-FoundationStereo model loaded and moved to GPU.")
    return dict(model=model, InputPadder=InputPadder, AMP_DTYPE=AMP_DTYPE)


def compute_stereo_depth_ffs(rect_left_bgr, rect_right_bgr, K_rect, baseline_m,
                              min_depth_m, max_depth_m, ffs_ctx):
    """FFS equivalent of compute_stereo_depth -- same (disparity, Z, valid) return shape."""
    import torch
    model, InputPadder, AMP_DTYPE = ffs_ctx["model"], ffs_ctx["InputPadder"], ffs_ctx["AMP_DTYPE"]

    # run_demo.py reads via imageio (RGB order); ours come from cv2 (BGR) -- convert to match.
    img0 = cv2.cvtColor(rect_left_bgr, cv2.COLOR_BGR2RGB)[..., :3]
    img1 = cv2.cvtColor(rect_right_bgr, cv2.COLOR_BGR2RGB)[..., :3]
    H0, W0 = img0.shape[:2]
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=AMP_DTYPE):
        if not FFS_HIERA:
            disp = model.forward(img0_t, img1_t, iters=FFS_VALID_ITERS, test_mode=True, optimize_build_volume="pytorch1")
        else:
            disp = model.run_hierachical(img0_t, img1_t, iters=FFS_VALID_ITERS, test_mode=True, small_ratio=0.5)
    disp = padder.unpad(disp.float())
    disp = disp.data.cpu().numpy().reshape(H, W).clip(0, None)

    K_scaled = K_rect.copy().astype(np.float64)
    K_scaled[:2] *= scale
    with np.errstate(divide="ignore", invalid="ignore"):
        Z = K_scaled[0, 0] * baseline_m / disp
    Z[~np.isfinite(Z)] = 0.0
    Z[disp <= 0] = 0.0

    if scale != 1.0:
        disp = cv2.resize(disp, (W0, H0), interpolation=cv2.INTER_NEAREST)
        Z = cv2.resize(Z, (W0, H0), interpolation=cv2.INTER_NEAREST)
...
def plot_disparity_and_depth(disparity, Z, valid, min_depth_m, max_depth_m,
    ...
    plt.title(disparity_label or "Disparity")
...
def main():
    ...
    # ---------- RGB stereo depth (Fast-FoundationStereo) ----------
    model_ckpt = check_weights()
    ffs_ctx = load_foundation_stereo_model(FFS_REPO_DIR, model_ckpt)
    disparity, Z, valid = compute_stereo_depth_ffs(
        rect_left, rect_right, P1[:3, :3], np.linalg.norm(T), MIN_DEPTH_M, MAX_DEPTH_M, ffs_ctx
    )
    plot_disparity_and_depth(disparity, Z, valid, MIN_DEPTH_M, MAX_DEPTH_M, "03",
                              disparity_label="Disparity (Fast-FoundationStereo)")
